    ...
    <header class="sticky top-0 z-50 w-full border-b ">
        <div class="flex h-16 items-center justify-between px-4 md:px-6">
            ...
            <div class="flex items-center space-x-3">
                <FormDialog title="Welcome back" description="Sign in to your SecureVault account" submit-text="Sign In"
                    :loading="isLoading" @submit="loginForm.handleSubmit">
                    <template #trigger>
                            ...
                            <template #default="{ field }">
                                <Field :data-invalid="isInvalid(field)">
                                    ...
                                    <Input :id="field.name" :name="field.name" :model-value="field.state.value"
                                        ...
                                        autocomplete="off" type="password" @blur="field.handleBlur"
                                        @input="field.handleChange($event.target.value)" />
                                    <FieldError v-if="isInvalid(field)" :errors="field.state.meta.errors" />
                                </Field>
                            </template>
                ...
                <FormDialog title="Create your account" description="Start securing your passwords today"
                    submit-text="Submit" :loading="isLoading" @submit="signUpForm.handleSubmit">
                    <template #trigger>
